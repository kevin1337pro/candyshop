...
## Update 2.3 — WebP, Mobile First und SEO (25.09.2026)

- Derselbe isolierte WordPress-/WooCommerce-/PHP-Stand wie bei 2.2: erneut alle 74 Regeln und die sechs Shopseiten/PLZ-AJAX geprüft. Keine echten Nachrichten oder Bestellungen an den Produktionsshop gesendet.
- Zusätzlich **33 SEO-/Bild-Prüfungen** erfolgreich: genau ein Canonical auf der Homepage mit tatsächlicher Basisdomain, Beschreibung, serverseitiger Essen-Infoblock, indexierbare Homepage bei freigegebener Sichtbarkeit, WebP-Auslieferung samt `srcset` und HTTP-Medientyp, interne Seiten/Filter/Demoprodukte mit `noindex`-Header, XML-Sitemap ohne interne Seiten/Autoren/Demoprodukte, Robots-Sitemap und freies Frontend-Crawling.
- Strukturierte Daten: ohne bestätigte Anschrift nur Organization; ausschließlich in der isolierten Testdatenbank mit bestätigter vollständiger Essener Testanschrift Store/PostalAddress und derselben sichtbaren Adresse. Ungültige Stadt verworfen. Verifizierungscode geprüft. Globale WordPress-Sichtbarkeit bleibt wirksam. Erkennung eines externen SEO-Plugins simuliert: keine doppelten Theme-Metadaten, interne `noindex`-Header bleiben erhalten. Keine vollständige Abnahme realer SEO-Drittplugins.
- Browser: natives Theme bei 320, 390, 768 und 1280 Pixeln ohne horizontalen Seitenüberlauf; Produktgitter 2/3/4 Spalten; PLZ-Prüfbereich auf dem ersten mobilen Bildschirm; Menü aufklappbar. Mobile Logo-Proportionen bleiben korrekt. React-Vorschau ebenfalls bei schmaler/Tablet-Breite geprüft; Produktmotive einzeln und proportioniert. Keine neuen Browserfehler im nativen Theme.
- Eingaben mindestens 16 px; zentrale Aktionsflächen mindestens 44 × 44 px einschließlich Rahmen. WebP-Breiten wechseln abhängig von Bildschirm/Pixeldichte und bereits geladenen Browserdateien. Das große Kampagnenbild ist priorisiert, nachfolgende Bilder sind lazy.
- Dateigrößen reproduzierbar in `docs/image-sizes.json`: 6.229.998 Bytes Originale gegenüber 457.406 Bytes für die größten sechs WebP-Motive (−92,7 %). Kein Lighthouse-Score und keine Ladezeit-/Ranking-Garantie daraus abgeleitet.
- TypeScript und Anwendungs-Lint erfolgreich. PNG-Originale liegen außerhalb von `public/` und des Docker-Kontexts. Bestehende eigene WordPress-Medien werden nicht automatisch umgeschrieben.

Indexierung, Rich Results und KI-Nennungen hängen nach dem Server-Update auch von tatsächlichen Betreiber-/Produktdaten, der Freigabe und externen Suchmaschinen ab. Keine Search-Console-/Bing-/Unternehmensprofil-Konten ohne Betreiber eingerichtet. Die früher dokumentierten Docker-/PayPal-/MariaDB-Grenzen bleiben bestehen.
